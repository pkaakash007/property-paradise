...
CREATE TABLE IF NOT EXISTS bookings (
  ...
  preferred_time TEXT,
  status TEXT DEFAULT 'pending', -- 'pending'|'confirmed'|'rescheduled'|'completed'|'cancelled'
  assigned_agent_id TEXT REFERENCES agents(id),
  notes TEXT,
  created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
...
-- Seed Locations
INSERT OR IGNORE INTO listing_locations (id, state, city, locality, area, pincode, address, latitude, longitude, privacy)
VALUES
...
('loc-5', 'Karnataka', 'Bangalore', 'Sadashivanagar', 'North Bangalore', '560080', '88 Royal Palms, Sadashivanagar, Bangalore', 13.0068, 77.5813, 'exact');

-- Seed Listings
INSERT OR IGNORE INTO listings (id, slug, title, description, property_type, listing_purpose, status, price, area_sqft, plot_area_sqft, builtup_area_sqft, bedrooms, bathrooms, floors, parking_spaces, facing, furnished_status, featured, verified, agent_id, location_id, primary_image_url, youtube_video_id)
VALUES
...
('listing-5', 'exclusive-sadashivanagar-gated-plot', 'Exclusive Sadashivanagar Gated Plot', 'Rare north-facing clear title plot in Bangalore''s most prestigious neighborhood. Ready for immediate villa construction.', 'plot', 'sale', 'featured', 45000000, 4800, 4800, 0, 0, 0, 0, 0, 'North', 'Unfurnished', 1, 1, 'agent-2', 'loc-5', 'https://images.unsplash.com/photo-1524813686514-a57563d77965?auto=format&fit=crop&w=1200&q=80', NULL);
...
('listing-5', 'https://images.unsplash.com/photo-1524813686514-a57563d77965?auto=format&fit=crop&w=1200&q=80', 'Sadashivanagar Plot', 1, 1);
